...
# Install system dependencies, browsers, and Xvfb
...
RUN EDGE_VERSION=$(microsoft-edge --version | awk '{print $3}' | cut -d '.' -f 1) \
...
CMD ["sh", "-c", "Xvfb :99 -screen 0 1920x1080x24 & robot --variable BROWSER:${BROWSER} --outputdir reports tests/"]